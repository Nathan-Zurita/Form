...
class AdminContatos {
    ...
    public function criar($dados) {
        try {
            ...
            $email = isset($dados['email']) ? htmlspecialchars(strip_tags($dados['email'])) : null;
    ...
    public function atualizar($id, $dados) {
        try {
            ...
            $query = "UPDATE " . $this->table_name . " 
                     SET nome = :nome, num_funcional = :num_funcional, telefone = :telefone, email = :email,
                         sexo = :sexo, origem = :origem, destino = :destino, updated = :updated 
                     WHERE id = :id";
     ...
     */
    public function buscarCombinacoes($filtroSexo = null) {
        try {
            $whereClause = '';
            if ($filtroSexo && in_array($filtroSexo, ['M', 'F'])) {
                $whereClause = "WHERE c1.sexo = :sexo AND c2.sexo = :sexo";
            }
            ...
            if ($filtroSexo && in_array($filtroSexo, ['M', 'F'])) {
                $stmt->bindParam(':sexo', $filtroSexo);
            }
            ...
            foreach ($contatos as $contato) {
                ...
                if ($match1 && $match2) {
                    ...
                    $combinacoes[] = [
                        ...
                        'data1' => $contato['data1'],
                        'data2' => $contato['data2'],
                        // IDs dos contatos para referência
                        'id1' => $contato['id1'],
                        'id2' => $contato['id2'],
    ...
    public function getEstatisticas() {
        try {
            ...
            $query = "SELECT origem, COUNT(*) as total FROM " . $this->table_name . " GROUP BY origem ORDER BY total DESC LIMIT 1000";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
            $stats['origens_populares'] = $stmt->fetchAll();
            
            // Destinos mais procurados (separar múltiplos destinos por vírgula)
            $query = "SELECT destino FROM " . $this->table_name . " WHERE destino IS NOT NULL AND destino != ''";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
            $destinos_raw = $stmt->fetchAll();
            
            $destinos_count = [];
            foreach ($destinos_raw as $row) {
                $destinos = explode(',', $row['destino']);
                foreach ($destinos as $destino) {
                    $destino = trim($destino);
                    if (!empty($destino)) {
                        if (!isset($destinos_count[$destino])) {
                            $destinos_count[$destino] = 0;
                        }
                        $destinos_count[$destino]++;
                    }
                }
            }
            
            // Ordenar por quantidade e pegar os top 1000
            arsort($destinos_count);
            $stats['destinos_populares'] = [];
            $count = 0;
            foreach ($destinos_count as $destino => $total) {
                if ($count >= 1000) break;
                $stats['destinos_populares'][] = [
                    'destino' => $destino,
                    'total' => $total
    ...
    public function buscarUnidades() {
        try {
            $query = "SELECT * FROM unidades WHERE ativo = 1 ORDER BY cidade, codigo";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
    ...
    private function verificarENotificarNovasCombinacoes($novoContatoId) {
        try {
            ...
            $novoContato = $this->buscarPorId($novoContatoId);
            if (!$novoContato) {
            ...
            $todasCombinacoes = $this->buscarCombinacoes();
            ...
            // Se encontrou combinações, enviar notificações automáticas completas
            if (!empty($novasCombinacoes)) {
                ...
                error_log("Total de " . count($novasCombinacoes) . " combinação(ões) encontrada(s) e notificações enviadas para contato ID {$novoContatoId} ({$novoContato['nome']})");
            } else {
                error_log("Nenhuma combinação encontrada para o novo contato ID {$novoContatoId} ({$novoContato['nome']})");
            }
    ...
    /**
     * Contar total de contatos no sistema
     */
    public function contarTotalContatos() {
        try {
            $query = "SELECT COUNT(*) as total FROM " . $this->table_name;
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
    ...
    /**
     * Contar contatos que possuem email cadastrado
     */
    public function contarContatosComEmail() {
        try {
            $query = "SELECT COUNT(*) as total FROM " . $this->table_name . " WHERE email IS NOT NULL AND email != ''";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return (int)$result['total'];
        } catch (PDOException $e) {
            error_log("Erro ao contar contatos com email: " . $e->getMessage());
            return 0;
        }
    }

    /**
     * Método público para reprocessar todas as combinações do sistema
     * Verifica se há novas combinações e envia notificações
     */
    public function reprocessarCombinacoes() {
        try {
            // Buscar todas as combinações atuais
            $combinacoes = $this->buscarCombinacoes();
            $totalCombinacoes = count($combinacoes);
            
            if ($totalCombinacoes > 0) {
                // Enviar notificações para todas as combinações encontradas
                $emailNotification = new EmailNotification();
                $sucessos = 0;
                $falhas = 0;
                ...
                foreach ($combinacoes as $combinacao) {
                    ...
                    if (isset($resultados['admin']) && $resultados['admin']['success']) $sucessos++;
                    if (isset($resultados['filiado1']) && $resultados['filiado1']['success']) $sucessos++;
                    if (isset($resultados['filiado2']) && $resultados['filiado2']['success']) $sucessos++;
                    
                    if (isset($resultados['admin']) && !$resultados['admin']['success']) $falhas++;
                    if (isset($resultados['filiado1']) && !$resultados['filiado1']['success']) $falhas++;
                    if (isset($resultados['filiado2']) && !$resultados['filiado2']['success']) $falhas++;
                }
                
                return [
                    'success' => true,
                    'novas_combinacoes' => $totalCombinacoes,
                    'emails_enviados' => $sucessos,
                    'emails_falharam' => $falhas,
                    'message' => "Processamento concluído: {$totalCombinacoes} combinações encontradas, {$sucessos} emails enviados com sucesso, {$falhas} falharam."
                ];
            } else {
                return [
                    'success' => true,
                    ...
                    'emails_enviados' => 0,
                    'emails_falharam' => 0,
                    'message' => 'Nenhuma combinação encontrada no sistema.'
                ];
            }
            
        } catch (Exception $e) {
            error_log("Erro ao reprocessar combinações: " . $e->getMessage());
            return [
                'success' => false,
                'novas_combinacoes' => 0,
                'emails_enviados' => 0,
